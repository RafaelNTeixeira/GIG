...
class RequesterSubmitedOffersStatus extends StatefulWidget {
  RequesterSubmitedOffersStatus({super.key});

  @override
  RequesterSubmitedOffersStatusState createState() => RequesterSubmitedOffersStatusState();
}

class RequesterSubmitedOffersStatusState extends State<RequesterSubmitedOffersStatus> {
  final searchController = TextEditingController();
  int _selectedIndex = 1;
  String? userUID;
  AppUser? currentUser;
  List<Map<String, dynamic>> acceptedOffersList = [];
  List<Map<String, dynamic>> declinedOffersList = [];
  ...
  Future<void> _fetchOffersList() async {
    ...
    if (mounted && currentUser != null) {
      final value1 = await DatabaseManagement.getSubmitedOffersByStatus(currentUser!.id, "Accepted");
      final value2 = await DatabaseManagement.getSubmitedOffersByStatus(currentUser!.id, "Declined");
      final value3 = await DatabaseManagement.getSubmitedOffersByStatus(currentUser!.id, "Finished");
      if (mounted) {
        setState(() {
          acceptedOffersList = value1;
          declinedOffersList = value2;
          finishedOffersList = value3;
        });
      }
    }
  }
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                ...
                Expanded(
                  child: Scrollbar(
                    child: declinedOffersList.length == 0 ? Text(
                      'No declined offers available',
                      style: TextStyle(
                          fontFamily: 'Poppins-Medium',
                          color: Colors.white,
                          fontSize: 15
                      ...
                      itemCount: declinedOffersList.length,
                      itemBuilder: (BuildContext context, int index) {
                        var offerData = declinedOffersList[index];
